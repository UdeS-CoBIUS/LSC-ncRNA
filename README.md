...
***LSC-ncRNA*** is an implementation of a sequence-based method that relies on the computation and the selection of common sequence motifs to provide a set of features for effectively classifying large ncRNAs families by a supervised learning approach in a reasonable time. 
...
Extract each family in a separate file such that the file name is Rfam family name as Rfxxxxx (xxxxx: is a number).
