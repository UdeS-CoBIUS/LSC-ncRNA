...
void multiple_alignment(string &dir_input, string &dir_output, string &test_name)
{
    ...
    for (auto & file_path : list_files_paths)
    {
        ...
        if(file_name == "info_test.csv") continue;
...
//    }
    ...
    for (auto & file_path : list_files_paths)
    {
        ...
        if(file_name.rfind("al_"+test_name,0)==0) {
            //command += "printf \"";
            command += "cmbuild --noss noss_";
...
void combine_cms_in_one_file(string &test_name) // files are in the same dirctory
{
    ...
    command += "cat noss_al_";
    command += test_name;
    command += "_*.cm > noss_cms_"+test_name+".cm";
...
int main(int argc, char** argv)
{
    ...
    start = chrono::high_resolution_clock::now();
    //cmbuild(test_name);
    cmbuild(dir_output,test_name);
    end = chrono::high_resolution_clock::now();
    time_calculation(start,end,"cmbuild --noss",test_name);
